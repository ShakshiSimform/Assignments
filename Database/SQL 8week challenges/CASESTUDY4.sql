...
SELECT region_id, COUNT(customer_id) AS allocated_cust
FROM customer_nodes
GROUP BY region_id
ORDER BY region_id;
...
SELECT ROUND(AVG(n.start_date - c.end_date), 1) AS avg_reallocation_days
FROM customer_nodes c
JOIN customer_nodes n USING(customer_id)
WHERE c.end_date < n.start_date
AND n.end_date != '9999-12-31';


--query 5 What is the median, 80th and 95th percentile for this same reallocation days metric for each region?
WITH CTE AS(
SELECT customer_id, region_name, (end_date - start_date) AS reloc
FROM customer_nodes
JOIN regions USING(region_id)
WHERE end_date != '9999-12-31'
)
SELECT region_name, PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY reloc) AS median,
    PERCENTILE_CONT(0.8) WITHIN GROUP (ORDER BY reloc) AS "80th_percentile",
    PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY reloc) AS "95th_percentile"
FROM cte
...
SELECT ROUND(AVG(deposit_count),2) AS avg_count, ROUND(AVG(deposit_amount),2) AS avg_amount
FROM CTE;
...
WITH CTE AS(SELECT customer_id, DATE_PART('MONTH', txn_date) AS months,  COUNT(CASE WHEN txn_type = 'deposit' THEN 1 END) AS deposit_count,
COUNT(CASE WHEN txn_type = 'purchase' THEN 1 END) AS purchase_count, COUNT(CASE WHEN txn_type = 'withdrawal' THEN 1 END) AS withdrawal_count
FROM customer_transactions
GROUP BY DATE_PART('MONTH', txn_date), customer_id
)
SELECT months, COUNT(DISTINCT customer_id) AS cust_count
FROM cte
...
WITH CTE AS(
	SELECT customer_id, DATE_PART('month', txn_date) AS months, 
...
SELECT cte.customer_id, CTE.months, SUM(cte.total_amount) OVER(PARTITION BY cte.customer_id ORDER BY cte.months) AS closing_balance
FROM CTE, customer_transactions
GROUP BY cte.months, cte.total_amount,cte.customer_id;


--query 5 What is the percentage of customers who increase their closing balance by more than 5%?
WITH CTE AS(
	SELECT customer_id, DATE_PART('month', txn_date) AS months, 
	SUM(CASE WHEN txn_type = 'deposit' THEN txn_amount ELSE -1 * txn_amount END) AS closing_balance
	FROM customer_transactions
	GROUP BY customer_id, DATE_PART('month', txn_date)
	),
	previous_balances AS (
    SELECT customer_id, months, 
	LAG(closing_balance) OVER (PARTITION BY customer_id ORDER BY months) AS previous_balance,closing_balance
    FROM cte
    ),
    increased_balances AS (
    SELECT customer_id
    FROM previous_balances
    WHERE closing_balance > previous_balance * 1.05
    GROUP BY customer_id
    )
SELECT ROUND(COUNT(DISTINCT ib.customer_id) * 100.0 / COUNT(DISTINCT ct.customer_id),2) AS percentage
FROM customer_transactions ct
LEFT JOIN increased_balances ib ON ct.customer_id = ib.customer_id;
...
SELECT customer_id, txn_date, txn_type, txn_amount, 
	SUM(CASE WHEN txn_type='deposit' THEN txn_amount ELSE -1*txn_amount
...
WITH CTE AS(
	SELECT customer_id, DATE_PART('month', txn_date) AS months, 
	SUM(CASE WHEN txn_type = 'deposit' THEN txn_amount ELSE -1 * txn_amount 
	END) OVER(PARTITION BY customer_id ORDER BY txn_date) AS total_amount
	FROM customer_transactions
	GROUP BY customer_id, DATE_PART('month', txn_date), customer_transactions.txn_type, 
	customer_transactions.txn_amount,customer_transactions.txn_date
	)
SELECT customer_id, MIN(total_amount) AS min_running_bal, ROUND(AVG(total_amount),2) AS avg_running_bal,
	MAX(total_amount) AS max_running_bal
FROM CTE
GROUP BY customer_id;
